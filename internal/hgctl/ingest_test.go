...
func TestClaudeSessionFilesWalksNestedDepths(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	shallow := mk(".claude/projects/proj/a.jsonl")
	deep := mk(".claude/projects/Users/zhwu/workspace/x/hourglass/b.jsonl")
	mk(".claude/projects/hgsmoke/c.jsonl")
	mk(".claude/projects/proj/notes.txt")
	got, err := claudeSessionFiles()
	if err != nil {
		t.Fatal(err)
	}
	set := map[string]bool{}
	for _, f := range got {
		set[f] = true
		if strings.Contains(f, "hgsmoke") {
			t.Errorf("hgsmoke not excluded: %s", f)
		}
		if !strings.HasSuffix(f, ".jsonl") {
			t.Errorf("non-jsonl included: %s", f)
		}
	}
	if !set[shallow] {
		t.Errorf("missing shallow file %s", shallow)
	}
	if !set[deep] {
		t.Errorf("missing DEEP file %s - recursive walk failed", deep)
	}
	if len(got) != 2 {
		t.Errorf("want 2 jsonl files, got %d: %v", len(got), got)
	}
}
