// Package ingest turns a machine's agent session transcripts into events.
//
// It is the single intake path, live and historical alike, and it is
// INCREMENTAL by file: a per-transcript ledger marker (emitted-turn cursor +
// byte size + time) means a session that keeps growing emits only its new turns,
// and one that stops emits nothing at all. Turns are never truncated - a delta
// is split into chunks at turn boundaries - so a first ingest streams the whole
// conversation in order and later growth appends to it.
//
// Two things here exist because a transcript store is not a well-behaved input:
// Codex opens a NEW rollout file on every fork or resume and replays the whole
// conversation into it, so thread tails let a fresh file skip history already
// emitted; and a transcript that parses to nothing (a workflow journal holding
// only started/result records) still gets a marker, because without one it stays
// eligible forever and the oldest-first parse budget never reaches a real
// session again.
package ingest
...
// Ingester carries only what intake needs: where things live and a clock the
// tests replace.
...
const (
	...
	// chunkBytes bounds one chunk-event body. A session's delta (its new,
	...
	// SyncLimit bounds how many grown sessions one scheduler-driven sync
	// re-ingests, so a sync stays within its context budget; the remainder is
	// picked up by the next run.
	// SyncLimit is how many new-or-grown transcripts one scheduled sync parses.
	SyncLimit = 8
)
...
// Mark records what was last ingested for one session: the transcript byte
...
// Mark is one transcript's ledger entry.
type Mark struct {
...
// Run gathers new-or-grown sessions and emits their deltas into the outbox.
// limit and parseCap of 0 mean unbounded, which is the first-install backfill.
func (i *Ingester) Run(id config.Identity, marks map[string]Mark, clients []string, limit, parseCap int, minInterval time.Duration) (int, bool, error) {
	var (
	...
	for _, c := range clients {
		got, skips, err := i.gatherSessions(c, marks, minInterval, parseCap)
		if err != nil {
	...
	for _, skip := range skipped {
		marks[skip.key] = Mark{Size: skip.size, Turns: 0, IngestedAt: now}
		changed = true
	}
	for _, cand := range candidates {
		...
		if start == n {
			...
			marks[key] = Mark{Size: cand.size, Turns: start, IngestedAt: now}
			changed = true
			continue
		}
		header := sessionHeader(cand.session)
		chunks := chunkDeltaTurns(header, start, cand.session.turns[start:n], chunkBytes)
		...
		for _, ch := range chunks {
			...
			if err := event.Enqueue(i.Paths.Outbox, ev); err != nil {
				errs = append(errs, err)
				failed = true
				break
			}
		...
		marks[key] = Mark{Size: cand.size, Turns: n, IngestedAt: now}
		if tKey != "" {
...
// each sync re-ingests up to SyncLimit new-or-grown sessions into the outbox
...
// Sync is the scheduler's bounded pass: at most SyncLimit new-or-grown
// transcripts per run, so one cycle stays inside its budget and the rest is
// picked up by the next.
func (i *Ingester) Sync(id config.Identity) error {
	clients, _ := Clients("all")
	marks, err := i.LoadLedger()
	if err != nil {
		return err
	}
	_, changed, ingestErr := i.Run(id, marks, clients, SyncLimit, SyncLimit, ingestMinInterval())
	if changed {
		if err := i.SaveLedger(marks); err != nil {
...
// Clients resolves the --client selector to the source list.
...
func replayedPrefix(mark Mark, turns []ingestTurn) int {
...
func appendThreadTail(mark Mark, tail string) Mark {
	for _, t := range mark.Tails {
		if t == tail {
			return mark
		}
...
func (i *Ingester) gatherSessions(client string, marks map[string]Mark, minInterval time.Duration, parseCap int) ([]ingestCandidate, []ingestSkip, error) {
	var (
		files   []string
...
func shouldReingest(mark Mark, size int64, now time.Time, minInterval time.Duration) bool {
...
func (i *Ingester) ingestedSessionsPath() string {
	return filepath.Join(i.Paths.Data, "ingested-sessions.json")
}
...
// LoadLedger reads the per-transcript marker ledger.
func (i *Ingester) LoadLedger() (map[string]Mark, error) {
	content, err := os.ReadFile(i.ingestedSessionsPath())
	...
	if err := json.Unmarshal(content, &marks); err == nil && marks != nil {
		normalized := make(map[string]Mark, len(marks))
	...
	if err := json.Unmarshal(content, &ids); err != nil {
		return nil, fmt.Errorf("parse ingest ledger %s: %w", i.ingestedSessionsPath(), err)
	}
	marks = make(map[string]Mark, len(ids))
	for _, id := range ids {
		marks[normalizeIngestKey(id)] = Mark{}
	}
	return marks, nil
}
...
// SaveLedger persists the marker ledger.
func (i *Ingester) SaveLedger(marks map[string]Mark) error {
	return fsx.WriteJSON(i.ingestedSessionsPath(), marks, 0o600)
}
